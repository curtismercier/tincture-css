...
const GLOBALS = resolve(ROOT, 'src/tenants/arzadon/globals.css');
const PROVIDER = resolve(ROOT, 'src/tenants/arzadon/components/ThemeProvider.tsx');
 ...
 * exposing the orthogonal axes for studio-designer + future moods.
 *
 ...
 *
 * AF-NEXT-10 (s01-084608) — never return null while mounting; tanks
 * Lighthouse perf. next-themes' default behaviour is correct here.
 */
...
if (!globals.includes('tincture/flavors.css')) {
  ...
  if (next === globals) {
    ...
    actions.push({ kind: 'patch', path: 'src/tenants/arzadon/globals.css', note: 'inserted @import "./tincture/flavors.css"' });
  }
}
...
if (!provider.includes('SurfaceFlavorWriter')) {
  ...
  actions.push({ kind: 'rewrite', path: 'src/tenants/arzadon/components/ThemeProvider.tsx', note: 'added SurfaceFlavorWriter, ember theme registered' });
}
